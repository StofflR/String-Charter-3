<template>
  <div class="mr-4 justify-center ml-4">
    ...
    <div class="w-full">
      <label for="route-select" class="text-1xl w-full font-extrabold bg-clip-text drop-shadow-lg">Available data
        sets:</label>
      <select id="route-select" class="form-control p-2 border border-gray-300 rounded-lg"
              @change="handleSelection($event)">
        <option value="" selected disabled>Choose Data Set:</option>
